...
function StartPoseGuide({ exerciseType, isMatching }: StartPoseGuideProps) {
  return (
    <div className="pointer-events-none absolute inset-0 z-20 flex flex-col items-center justify-center px-5 pt-20 pb-24">
      <div
        className={`w-full max-w-xl rounded-3xl border-2 border-dashed p-3 transition-colors duration-200 ${
          isMatching
      ...
      <p className="bg-camera-scrim text-body mt-3 rounded-full px-5 py-2 text-center font-semibold">
        {isMatching ? "좋아요! 운동 판정을 시작할게요" : GUIDE_MESSAGE[exerciseType]}
      </p>
      {!isMatching && (
        <p className="text-body-small mt-2 text-center text-white/80">
